...
```{asis}
#mean of life expectancy by country for countries whose pop is below the mean for the dataset
...
#whether life expectancy is smaller than 50 and larger than 70
lower_threshold = 50
upper_threshold = 70
...
```{asis}
### Answer
vector_info = function(x, med=TRUE){
...
cor.test(log(gapminder$gdpPercap), log(gapminder$lifeExp))
plot(log(gapminder$gdpPercap), log(gapminder$lifeExp), col="orange", cex=0.5)

#correlation by year
for (i in unique(gapminder$year)){
  print(cor.test(log(gapminder$gdpPercap[gapminder$year==i]), log(gapminder$lifeExp[gapminder$year==i])))
  #plot(log(gapminder$gdpPercap), log(gapminder$lifeExp),    col="orange", cex=0.5)
}
```
